...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(11, 11));
    ...
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(14, 14, 16), 3, 15, 2);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(38, 38, 42), 3, 15, 1);

    if (*v) {
        window->DrawList->AddCircleFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + (check_bb.Max.y - check_bb.Min.y) / 2), 2, GetColorU32(ImGuiCol_CheckMark));
    }

    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Min.x + 20, check_bb.Min.y + 1), label);
    return pressed;
}
...
bool ImGui::tab(const char* label, bool selected)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 size = CalcItemSize(ImVec2(75, 35), 50 + style.FramePadding.x, label_size.y + style.FramePadding.y);
    const ImRect bb(pos, pos + size);
    ItemSize(size, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
    ...
    int a = int(t * 255);
    auto text_color = ImGui::GetColorU32(ImLerp(ImVec4(255 / 255.f, 255 / 255.f, 255 / 255, 255 / 255.f), ImVec4(0 / 255.f, 183 / 255.f, 255 / 255.f, 255.f / 255.f), t));
    draw->AddRectFilled(ImVec2(bb.Max.x, bb.Min.y + 2), bb.Min, ImColor(0, 183, 255, a));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_CheckMark : ImGuiCol_CheckMark), style.GrabRounding);
