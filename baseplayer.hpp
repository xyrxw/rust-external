...
class c_baseplayer {
public:
	void mounted(bool toggle);
	void set_rapid(bool toggle);
...
public:
	...
	{
		this->player = read<uintptr_t>(ent + 0x28); //BasePlayer
		this->baseentity = read<uintptr_t>(ent + 0x20); //BaseEntity
...
inline std::unique_ptr<std::vector<baseEntity>> player_list = std::make_unique<std::vector<baseEntity>>();
