...
void c_baseplayer::antiaim(bool toggle)
{
	uintptr_t player_input = read<uintptr_t>((uintptr_t)this + 0x4E0);
	auto state = read<uintptr_t>(player_input + 0x20); //public InputState state;
	auto current = read<uintptr_t>(state + 0x10); //public InputMessage current;
	if (toggle)
	{
		write<Vector3>(current + 0x18, Vector3(rand() % 999 + -999, rand() % 999 + -999, rand() % 999 + -999)); //Body->PlayerInput
		write<Vector3>(current + 0x58, Vector3(rand() % 999 + -999, rand() % 999 + -999, rand() % 999 + -999)); //head->PlayerInput
		write<Vector3>(current + 0x70, Vector3(rand() % 999 + -999, rand() % 999 + -999, rand() % 999 + -999)); //public Vector3 aimAngles;
	}
}

void c_baseplayer::mounted(bool toggle)
{
	uintptr_t mountable = read<uint64_t>((uintptr_t)this + 0x600);
	if (mountable)
	{
		DWORD64 wield = read<bool>((uintptr_t)this + 0x2B0);
		write<bool>(wield, true);
	}
}
void c_baseplayer::set_rapid(bool toggle)
{
	...
	if (toggle)
	{
		...
		write<float>(base_projectile + 0x1f4, vars.rapid_val);
	}
}
void c_baseplayer::set_recoil(bool toggle) {
	...
	uintptr_t held = GetHeldItem((uintptr_t)this);
	uintptr_t base_projectile = read<uintptr_t>(held + 0x98);
	uintptr_t recoil = read<uintptr_t>(base_projectile + 0x2D8); // public RecoilProperties recoil; //2d8
	if (toggle)
	{
		...
		write<bool>(base_projectile + 0x288, true);
	}
...
void c_baseplayer::set_flag(int flag, bool toggle) {
	if (toggle)
	{
		auto player_flags = read<uintptr_t>((uintptr_t)this + 0x680);
		player_flags |= flag;
		write<uintptr_t>((uintptr_t)this + 0x680, player_flags);
	}
}
...
void c_baseplayer::set_instanteoka(bool toggle) {
	if (localPlayer2.getHeldItem().name.find("eoka") != std::string::npos && toggle)
	{
		uintptr_t held = GetHeldItem((uintptr_t)this);
		uintptr_t base_projectile = read<uintptr_t>(held + 0x98);
